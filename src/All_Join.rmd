...
# ais_mmsi <- read.csv("data/ais_mmsi.csv") %>%
#   as_tibble(.name_repair = 'universal') %>% 
#   rename(AIS.MMSI = mmsi)
# ais_yearly_distance <- read_feather("data/AIS_yearly_distance.feather") %>% 
#   as_tibble(.name_repair = 'universal')
AIS_df <- read.csv("data/AIS_yearly_distance.csv") %>% 
  as_tibble(.name_repair = 'universal') %>% 
  rename(MMSI = mmsi)
# load("data/ais_mmsi.Rda")
load("data/MRV.Rda")
```
...
# Combine bulkers and container ships
```{r}
fleet_df <- bind_rows(bulkers_df, containerships_df)
```
...
AIS_df <- AIS_df %>% 
  arrange(AIS.year) %>% 
  pivot_wider(names_from = AIS.year,
              names_prefix = "AIS.Distance.",
              values_from = AIS.distance,)
```
...
fleet_df <- fleet_df %>% 
  ...
  relocate(contains("EU.Distance"), .after = AIS.Distance.2021) %>%
  relocate(contains("EIV"), .after = Design.Efficiency) %>% 
  relocate(contains("EEDI"), .after = MRV.EIV.2020)
```
...
```
44% of ships report in MRV. This is quite high!
