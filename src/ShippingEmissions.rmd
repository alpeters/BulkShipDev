...
# AIS Calculations for Engineering Fuel Consumption Calculations



# AIS Calculations
```{r, eval = FALSE}
py_run_file('EEZ_EU.py')
```
...
```{r, eval = FALSE}
py_run_file('AIS_Calculations_EU.py')
```
```{r, eval = FALSE}
py_run_file('AIS_Calculations_TripStats.py')
```

```{r, child = 'AIS_Calculations_TripStats_Load.rmd', eval = FALSE}
```
# Data Joining
...
# Regressions
```{r, child = 'Efficiency_Regression.rmd'}
```
