...
Function for quick summary tables
```{r include = FALSE}
kable_titles <- function(df, format = "simple"){
  kable(df,
        format = format,
        col.names = colnames(df) %>% str_replace_all("\\.", " ") %>% str_to_sentence())
}

# initial_tidy <- function(df){
...
```{r}
# MRV_df <- read_MRV("data/MRV/2018-v249-13052021-EU MRV Publication of information.xlsx")
MRV_df <- read_MRV("src/data/MRV/2018-v265-30042022-EU MRV Publication of information.xlsx")

# temp_df <- read_MRV("data/MRV/2019-v163-20052021-EU MRV Publication of information.xlsx")
temp_df <- read_MRV("src/data/MRV/2019-v204-19052022-EU MRV Publication of information.xlsx")

MRV_df <- MRV_df %>% 
  bind_rows(temp_df)

# temp_df <- read_MRV("data/MRV/2020-v63-05102021-EU MRV Publication of information.xlsx")
temp_df <- read_MRV("src/data/MRV/2020-v150-19052022-EU MRV Publication of information.xlsx")

temp_df <- temp_df %>% 
  rename("Annual total time spent at sea [hours]" = "Annual time spent at sea [hours]",
         "Total time spent at sea [hours]" = "Time spent at sea [hours]")

MRV_df <- MRV_df %>% 
  bind_rows(temp_df)

temp_df <- read_MRV("src/data/MRV/2021-v41-25082022-EU MRV Publication of information.xlsx")
...
```{r}
MRV_df <- MRV_df %>%
  mutate(across(c(port.of.registry,
                  home.port),
                str_to_title)) %>%
  mutate(across(c(reporting.period,
                  ship.type,
                  port.of.registry,
                  home.port,
                  ice.class),
...
```{r}
MRV_df <-MRV_df %>% 
  mutate(technical.efficiency = technical.efficiency %>% 
                                str_remove_all("Not\\sApplicable|\\(|\\)") %>% 
                                str_trim(),
         technical.efficiency = if_else(str_starts(technical.efficiency, "[:digit:]"), 
                                        paste("NA", technical.efficiency),
                                        technical.efficiency)) %>% 
  separate(technical.efficiency,
           c("efficiency.metric", "efficiency.value", "efficiency.unit"),
           sep = " ",
           extra = "merge",
           fill = "right",
           remove = FALSE) %>%
  mutate(efficiency.metric = na_if(efficiency.metric, "NA"),
...
```{r}
save(MRV_df, file = "src/data/MRV.Rda")
```
