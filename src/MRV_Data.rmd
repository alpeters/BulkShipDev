...
Distance travelled, speed, emission factor
```{r}
MRV_df <- MRV_df %>% 
  mutate(EU.distance = total.fc / fc.per.distance * 1E3) %>% 
  mutate(speed = EU.distance / time.at.sea) %>% 
  mutate(emission.factor = total.co2 / total.fc)
```
