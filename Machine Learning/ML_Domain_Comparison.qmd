...
filenames_df <- tribble(
  ~target, ~transform,   ~models,        ~set, ~split_feature1,                ~splits1, ~split_feature2,         ~splits2,     ~csv_file,   ~csv_file_within,
     ...
     'fc',           '',       "",      'work',   'relseaspeed',    paste0("quart", 1:4),              NA,               NA,     'test_fc',     'train_target',
     'fcnorm', 'linear', "struct",   'struct',   'relseaspeed',    paste0("quart", 1:4),              NA,               NA,     'test_fc',         'train_fc',
)
...
stat_plot_model(
  ...
  "transfer_error",
  "mae",
  c("eng", "gb", "lasso", "linear", "rf", "ridge")
) |> 
  print()
```
...
metric_table |> 
  ...
  group_by(target, split, vars) |> 
  filter(within_error == min(within_error)) |> 
  select(target, model, split, vars, within_error) |> 
