...
for(set in sets){
  ...
  for(quart in 1:4){
    df <- read_csv(file.path(mltrackeddatapath,
      ...
      mutate(set = set,
             quartile = quart)
    metric_table_fc <- bind_rows(metric_table_fc, df)
  }
  
}

metric_table_fc <- metric_table_fc |> 
  mutate(target = 'fc')
```
...
for(set in sets){
  ...
  for(quart in 1:4){
    df <- read_csv(file.path(mltrackeddatapath,
      ...
      mutate(set = set,
             quartile = quart)
    metric_table_fe <- bind_rows(metric_table_fe, df)
  }
  
}

metric_table_fe <- metric_table_fe |> 
  mutate(target = 'fe')
```


```{r}
metric_table <- bind_rows(metric_table_fc, metric_table_fe) |> 
  mutate(set = paste0(set, '_', target)) |>
