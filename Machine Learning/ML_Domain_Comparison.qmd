...
best_stat_function <- function(stat, value){
  if (stat %in% c('mae', 'mape', 'rmse', 'ate')) {
    ...
    stop(paste(stat, "is an unknown statistic"))
  }
}
```

```{r}
# stat_plot_best <- function(
#     metric_table,
#     trans_error_stat = "transfer_error",
#     pred_error_stat = "mae",
#     models = "all",
#     ylims = c(NA, NA)
#   ){
...
stat_plot_model <- function(
  ...
  stat_plot <- metric_table |> 
    ...
    scale_color_manual(values = pres_palette) +
    theme_pres(base_size) +
    labs(y = str_to_upper(pred_error_stat),
         x = 'Test split',
         fill = 'Feature set',
         color = 'Feature set') +
    coord_cartesian(ylim = ylims) +
...
metric_table <- metric_table |>
  ...
  mutate(min_within_error = min(within_error)) |> 
