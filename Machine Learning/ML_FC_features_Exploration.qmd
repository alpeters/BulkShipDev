...
```{r}
model_names_df <- read_results('cal_fc') |> 
...
```{r}
variants_calc <- c('cal_fc',
                   paste0('disjoint_b', 2:5),
                   'calc_dist',
                   paste0('disjoint_b', 7:9)
                   )

names(variants_calc) <- read_csv(file.path(mltrackeddatapath, 'ML_FC_variants.csv'), show_col_types = FALSE) |> 
  select(variable, disjoint_b, disjoint_b_name) |> 
