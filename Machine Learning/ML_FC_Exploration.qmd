...
params <- paste0(str_remove(fileprefix, '_Exploration'), 'F', set, '_eval_fc_stats.csv') %>%
  ...
  drop_na(Parameters) |> 
  mutate(Parameters = ifelse(Parameters=='', 'n/a', Parameters)) |> 
  arrange(Model)
...
# rows_rule_below <- rows_rule_below[rows_rule_below != nrow(params)]
rows_rule_below
```
...
params |> 
  ...
  collapse_rows(columns = 1,
                ...
                valign = 'top') |>
  row_spec(1,
           extra_latex_after = "\\arrayrulecolor{black!30}\\midrule") |> 
  row_spec(nrow(params),
